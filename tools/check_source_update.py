...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Path to the current working translation JSON, for example working/Game.bg.json",
    )
    parser.add_argument(
        "--source-json",
        required=True,
        type=Path,
        help="Path to a newly extracted source Game.json, for example source/Game.en.json",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=20,
        help="How many sample differences to print for each category",
    )
    return parser
