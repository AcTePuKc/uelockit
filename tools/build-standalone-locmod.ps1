param(
    [string]$PackageName = "MyUnrealGame-LocMod_P",
...
if ($PackageName -eq "MyUnrealGame-LocMod_P") {
...
$locresOutput = Join-Path $WorkspaceRoot ("output\{0}.{1}.locres" -f $LocalizationTargetName, $CultureFolder)
...
& $locresBuildScript -TranslationFormat $TranslationFormat -WorkspaceRoot $WorkspaceRoot -ConfigPath $ConfigPath -LocalizationTargetName $LocalizationTargetName -CultureTag $CultureFolder
